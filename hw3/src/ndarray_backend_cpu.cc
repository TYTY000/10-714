...
enum class index_mode { GET, SET, SET_VAL };

void index_striding(const AlignedArray *in, AlignedArray *out,
                    std::vector<int32_t> shape, std::vector<int32_t> strides,
                    size_t offset, index_mode mode, int val = -1) {
  int dim = shape.size();
  std::vector<int32_t> loop(dim, 0);
  int cnt = 0;
  while (true) {
    int index = offset;
    for (int i = 0; i < dim; i++) {
    ...
    switch (mode) {
    case index_mode::GET:
      out->ptr[cnt++] = in->ptr[index];
      break;
    case index_mode::SET:
      out->ptr[index] = in->ptr[cnt++];
      break;
    case index_mode::SET_VAL:
      out->ptr[index] = val;
      break;
    }
...
void Compact(const AlignedArray &a, AlignedArray *out,
   ...
   */
  /// BEGIN SOLUTION
  index_striding(&a, out, shape, strides, offset, index_mode::GET);
  /// END SOLUTION
}
...
void EwiseSetitem(const AlignedArray &a, AlignedArray *out,
   ...
   */
  /// BEGIN SOLUTION
  index_striding(&a, out, shape, strides, offset, index_mode::SET);
  /// END SOLUTION
}
...
void ScalarSetitem(const size_t size, scalar_t val, AlignedArray *out,
  ...
  /// BEGIN SOLUTION
  index_striding(nullptr, out, shape, strides, offset, index_mode::SET_VAL,
                 val);
  /// END SOLUTION
}
